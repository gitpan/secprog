...
=item * Safe sequence numbers

=item * Dynamic method names
...
This is the list of topics I'll be talking about
...
the issues I've missed out, and I have missed out a lot.



=head1 Introduction

=head2 Don't trust the user

=over 4

=item * Implicit trust -> eventual disaster

=item * No trust -> more safety
...
=item * Check return codes of system calls

  open STDOUT,'> write-protected';

=item * Handle "impossible cases" explicitly
...
instance, failing to open a file on an existing filehandle will
actually leave the previous file open. In the example here, any
data printed to STDOUT will go to the program's original output,
rather than to the file. Most programs do check the status of
opening files, but everything after that is often just assumed to
work. If the secure operation of your program relies on files being
written to correctly, you should check the return status of print(),
write(), and even close().
...
All of these measures are much easier to implement if you start out
using them. Retrofitting strict or taint checking to an existing
program is painful.
...
=item * Data from external sources is tainted

  $^X  @ARGV  %ENV
  <>  read()  readline()  `` qx()    etc
  shmread()  msgrcv()  getpw*()      etc

=item * All data derived from tainted data is also tainted

=item * Tainted data can't affect external sources
...
=item * C<$1>, C<$2>, etc are (almost) never tainted
...
Of course, quite often, we do want the user's input to affect
external entities, so Perl provides a simple way to untaint
values. The regex capture variables, $1, $2 and so on are almost
always untainted. This forces you to validate user input with a
regex before treating it as safe, but doesn't actually force the
use of a safe pattern. Perl will happily untaint $1, even if you
used .* as your pattern, and treated all values as safe. So we'll
have a look at validation in a couple of slides.

I said capture variables were *almost* always untainted. If "use
locale" is in effect, then regex captures are tainted, since locales
can affect the meaning of some regex metacharacters. Also, the "re"
pragma has a 'taint' option which causes regex captures to be tainted
...
=item * Sanitise the environment

  %ENV=( PATH => '/bin:/usr/bin' );

=item * Additional C<%ENV> taint checks

  PATH IFS CDPATH ENV BASH_ENV

=item * Don't invoke the shell
...
=item * Safe, but invokes the shell
...
=item * Allow safe; don't disallow unsafe

  die "Bad!" if /[;<>&^"']/;     # Bad!

  die "Bad!" unless /^[\w\s]+$/; # Good

=item * Check allowed ranges
...
but Perl doesn't treat nulls specially. If a user manages to
specify a filename containing an embedded null, the addition of
a .safe extension won't make any difference. If that's the only
check you make on filenames, you've just allowed the user to view
every file on your system. Checking for null bytes makes the safe
extension technique safer, but there are better ways of restricting
filename choices.
...
=item * C<< open FH,"< $fname\0"; >>

No magic

=item * C<< my $fh=IO::File->new($fname,'<'); >>

No magic, lexical filehandle

=item * C<< open my $fh,'<',$fname; >>
...
they will probably have more access than you wanted if you open the
file as specified. Ideally, a user shouldn't be able to specify such
a file, but a secure application should protect itself at all levels.
...
Specifying a single leading space and trailing null byte turns off
...
=item * Hidden fields aren't secure

  <input type="hidden" name="password"
    value="secret">

=item * Expect multiple values
...
=item * Mostly like hidden fields

=item * Message digest adds security
...
=item Let the database do it for you

  my $seqn=$dbh->selectall_arrayref(q(
    select nextval('name_of_sequence')
  ))->[0][0];

=item Or do it yourself
...
This slide was added at the last minute, so it's not as well thought
out as it could be.

The first example of sequence number generation here is pretty simple
- just find the biggest existing number, and add one. However, if
two processes execute this code at the same time, they will both
generate the same number. You could make this safer by running
it in a serialised isolation level, but you need to serialise the
whole transaction for that to work, which won't help performance.
...
your own "increment" operation, which could, for the sake of
argument, generate a Fibonacci sequence or, more usefully, passes
the raw sequence number through a message digest function to produce
a more random-looking sequence.
...
=item * Works if C<$meth> is a method name

  my $meth = 'do_stuff';
  $obj -> $meth ();     # $obj->do_stuff()

=item * Works across packages

  my $meth = 'Other::Pkg::do_stuff';
  $obj -> $meth ();

=item * Taint checks don't protect

=item * Restrict the user's choice
...
As well as nonexistent methods, users could specify methods which
are only suitable for internal use, or which would allow more access
than appropriate.
...
=item * But do it carefully

=item * It's not that hard

=back

=cut

So, as I said at the beginning, don't trust the user to do things
sensibly, even if that's what they were intending. For your program
to be secure, you need to check everything they enter, and even then,
still be careful what you do with it. However, if you write your
code that way from the start, it's not too onerous, and shouldn't
add much to your development time.
...
=item http://www.dwheeler.com/secure-programs/

Secure programming for Linux/Unix

=item S< >

=item http://www.w3.org/Security/Faq/

WWW Security FAQ

=item S< >

=item C<perldoc perlsec>

Perl documentation on tainting

=item S< >

=item $CPAN/authors/id/P/PM/PMH/secprog-tpc2002.tgz
